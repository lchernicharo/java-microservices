...
import br.com.tech4me.primeirorest.view.model.AnimalRequestModel;
import br.com.tech4me.primeirorest.view.model.PessoaModeloCompletoResponse;
...
public class PessoaController {
    ...
    @PostMapping(value="/{dono}/animal")
    public ResponseEntity<Void> adicionarAnimal(@RequestBody @Valid AnimalRequestModel animal,
                                                    @PathVariable String dono) {
        ResultadoEnvioMensagemAsync result = service.adicionarAnimal(dono, new ModelMapper().map(animal, Animal.class));

        if(result == ResultadoEnvioMensagemAsync.MENSAGEM_ENVIADA_COM_SUCESSO) {
    ...
    @GetMapping(value="/{id}")
    public ResponseEntity<PessoaModeloCompletoResponse> obterPorId(@PathVariable String id) {
        Optional<PessoaDto> pessoa = service.obterPorId(id);

        if(pessoa.isPresent()) {
            PessoaDto dto = pessoa.get();

            dto.setAnimais(service.obterAnimaisPorPessoa(id));

            return new ResponseEntity<>(
                new ModelMapper().map(dto, PessoaModeloCompletoResponse.class), 
                HttpStatus.OK
            );
        }
